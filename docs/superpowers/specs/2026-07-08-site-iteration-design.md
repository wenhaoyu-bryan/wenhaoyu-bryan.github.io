...
5. Agent 平台产品设计（0→1）—— 泛化的 agentic 设计模式（双模态、分层资产、
   对话生成技能、治理、自动化编排），不出现任何产品/公司/行业名
6. 编码之外 —— 双轨知识库、MCP、企业 AI 培训
7. 能力边界 —— 诚实写"熟练什么、边界在哪、下一步"，符合 engineering-honest 文风
...
必须隐去并泛化：雇主与公司名、内部/私有项目名、自建工具名、具体业务与客户
细节、培训对象的公司信息、内部数据规模的精确数字（可用"80+"等模糊表述）、
部署位置。模型只写产品线不写版本号。涉及隐私的条目宁可整条删除，不做泛化保留。
